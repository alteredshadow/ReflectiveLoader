...
// Objective-C runtime function for selector fixup
...
static void* custom_tlv_get_addr(struct tlv_descriptor* d) {
    void* storage = pthread_getspecific((pthread_key_t)d->key);
    if (storage == NULL) {
        TLVImageInfo* info = findTLVImageInfo((pthread_key_t)d->key);
        if (!info)
            dyld::throwf("TLV access for unknown pthread key %lu", d->key);
        size_t sz = info->totalSize;
        if (sz == 0) sz = sizeof(void*);  // safety floor
        storage = calloc(1, sz);
        if (!storage)
            dyld::throwf("TLV storage allocation failed (%zu bytes)", sz);
        if (info->initialDataSize > 0)
            memcpy(storage, info->initialData, info->initialDataSize);
        pthread_setspecific((pthread_key_t)d->key, storage);
    }
...
void ImageLoaderMachO::fixupObjCSelectors(const LinkContext& context)
{
	// Fix up Objective-C selector references to point to canonical runtime selectors
	const uint32_t cmd_count = ((macho_header*)fMachOData)->ncmds;
	const struct load_command* const cmds = (struct load_command*)&fMachOData[sizeof(macho_header)];
	const struct load_command* cmd = cmds;
	...
	for (uint32_t i = 0; i < cmd_count; ++i) {
		if ( cmd->cmd == LC_SEGMENT_COMMAND ) {
			...
			for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
				// Fix up __objc_selrefs section
				if ( strcmp(sect->sectname, "__objc_selrefs") == 0 ) {
					const char** selRefs = (const char**)(sect->addr + fSlide);
					const size_t count = sect->size / sizeof(char*);

					dyld::log("dyld: fixing up %zu Objective-C selectors in %s\n", count, this->getPath());

					for (size_t j = 0; j < count; ++j) {
						const char* selName = selRefs[j];
						if (selName != NULL) {
							// Register selector with runtime and get canonical selector
							void* canonicalSel = sel_registerName(selName);
							// Update reference to point to canonical selector
							selRefs[j] = (const char*)canonicalSel;

							if (context.verboseInit) {
								dyld::log("  selector[%zu]: %s -> %p\n", j, selName, canonicalSel);
							}
						}
					}
				}
...
{
	// Fix up Objective-C selectors before running initializers
	fixupObjCSelectors(context);
				...
				for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
					...
					if ( type == S_MOD_INIT_FUNC_POINTERS ) {
						...
						for (size_t j=0; j < count; ++j) {
							...
							dyld::log("dyld: about to call initializer #%zu at %p in %s\n", j, func, this->getPath());
							...
							dyld::log("dyld: initializer #%zu completed successfully\n", j);
...
{
    bool hasFlag = (this->machHeader()->flags & MH_HAS_TLV_DESCRIPTORS) != 0;

    // Scan sections for TLV-related types regardless of flag —
    // some toolchains (e.g. Go) may emit __thread_vars without setting MH_HAS_TLV_DESCRIPTORS.
    ...
    if (!tvAddr || tvSize == 0) {
        if (hasFlag)
            dyld::log("dyld: TLV: MH_HAS_TLV_DESCRIPTORS set but no __thread_vars section found in %s\n", this->getPath());
        return;
    }

    if (!hasFlag)
        dyld::log("dyld: TLV: MH_HAS_TLV_DESCRIPTORS not set but __thread_vars found in %s — patching anyway\n", this->getPath());
    ...
    node->info = { tlvKey, initCopy, tdSize, allocSize };
...
void ImageLoaderMachO::mapSegments(const void* memoryImage, uint64_t imageLen, const LinkContext& context)
{
	...
	for(unsigned int i=0, e=segmentCount(); i < e; ++i) {
		...
		vm_size_t fileSize = segFileSize(i);
		vm_size_t vmSize = segSize(i);

		// Copy file content
		if (fileSize > 0) {
			kern_return_t r = vm_copy(mach_task_self(), srcAddr, fileSize, loadAddress);
			if ( r != KERN_SUCCESS )
				throw "can't map segment";
		}

		// Zero-fill trailing BSS if segment is larger than file size
		if (vmSize > fileSize) {
			vm_address_t bssStart = loadAddress + fileSize;
			vm_size_t bssSize = vmSize - fileSize;
			if ( context.verboseMapping )
				dyld::log("  zero-filling BSS at 0x%08lX, size 0x%lX\n", (uintptr_t)bssStart, (uintptr_t)bssSize);
			bzero((void*)bssStart, bssSize);
		}

		if ( context.verboseMapping )
			dyld::log("%18s at 0x%08lX->0x%08lX (fileSize=0x%lX, vmSize=0x%lX)\n",
					 segName(i), (uintptr_t)loadAddress, (uintptr_t)loadAddress+vmSize-1,
					 (uintptr_t)fileSize, (uintptr_t)vmSize);
        
        /*
        if (mlock((void *)loadAddress, size) != 0) {
                perror("OMG: mlock failed");
        } else {
                printf("OMG: Page successfully locked in RAM.\n");
        }
        */
