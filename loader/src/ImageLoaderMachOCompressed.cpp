															...
															unsigned int segCount, unsigned int libCount, const LinkContext& context)
{
	...
	try {
		...
		// vmcopy segments
        //printf("invoking 'image->mapSegments' \n");
        ...
        //printf("instantiateFromMemory: catch\n");
		// ImageLoader::setMapped() can throw an exception to block loading of image
		// <rdar://problem/6169686> Leaked fSegmentsArray and image segments during failed dlopen_preflight
		delete image;
		throw;
    ...
    //printf("instantiateFromMemory: %p\n", image);
...
#else // UNSIGN_TOLERANT
void ImageLoaderMachOCompressed::doApplyFixups(const LinkContext& context, const dyld_chained_fixups_header* fixupsHeader)
{
	dyld::log("dyld: applying chained fixups to %s\n", this->getPath());

	const macho_header* mh = (const macho_header*)machHeader();
	const uint8_t* linkEditBase = fLinkEditBase;

	// Get the chained starts structure
	const dyld_chained_starts_in_image* startsInImage =
		(dyld_chained_starts_in_image*)((uint8_t*)fixupsHeader + fixupsHeader->starts_offset);

	// Get symbols string table
	const char* symbolsPool = (char*)fixupsHeader + fixupsHeader->symbols_offset;

	if (context.verboseBind) {
		dyld::log("dyld: chained fixups: version=%u, imports_count=%u, imports_format=%u\n",
				 fixupsHeader->fixups_version, fixupsHeader->imports_count, fixupsHeader->imports_format);
	}

	// Build array of resolved target addresses for each import
	uintptr_t* targetAddrs = (uintptr_t*)alloca(sizeof(uintptr_t) * fixupsHeader->imports_count);

	// Parse imports table based on format
	const uint8_t* importsStart = (uint8_t*)fixupsHeader + fixupsHeader->imports_offset;
	...
	for (uint32_t i = 0; i < fixupsHeader->imports_count; ++i) {
		...
		switch (fixupsHeader->imports_format) {
			case DYLD_CHAINED_IMPORT: {
				const dyld_chained_import* import = ((const dyld_chained_import*)importsStart) + i;
				libOrdinal = import->lib_ordinal;
				weakImport = import->weak_import;
				symbolName = symbolsPool + import->name_offset;
				break;
			}
			case DYLD_CHAINED_IMPORT_ADDEND: {
				const dyld_chained_import_addend* import = ((const dyld_chained_import_addend*)importsStart) + i;
				libOrdinal = import->lib_ordinal;
				weakImport = import->weak_import;
				symbolName = symbolsPool + import->name_offset;
			...
			case DYLD_CHAINED_IMPORT_ADDEND64: {
				const dyld_chained_import_addend64* import = ((const dyld_chained_import_addend64*)importsStart) + i;
				libOrdinal = import->lib_ordinal;
				weakImport = import->weak_import;
				symbolName = symbolsPool + import->name_offset;
				addend = import->addend;
			...
			default:
				dyld::throwf("unsupported chained import format: %d", fixupsHeader->imports_format);
		}

		// Resolve the symbol
		const ImageLoader* targetImage = nullptr;
		uint8_t symbolFlags = weakImport ? BIND_SYMBOL_FLAGS_WEAK_IMPORT : 0;
		try {
			uintptr_t symbolAddress = this->resolve(context, symbolName, symbolFlags, libOrdinal, &targetImage, nullptr, true);
			targetAddrs[i] = symbolAddress + addend;

			if (context.verboseBind) {
				dyld::log("dyld: chained fixup: %s -> 0x%lx (from %s)\n",
						 symbolName, targetAddrs[i], targetImage ? targetImage->getShortName() : "self");
			}
		}
		catch (const char* msg) {
			const char* expectedLib = (libOrdinal > 0 && libOrdinal <= (int)this->libraryCount())
				? this->libPath(libOrdinal) : "unknown";
			dyld::log("dyld: ERROR resolving symbol #%u: '%s' (lib ordinal %d: %s): %s\n",
					 i, symbolName, libOrdinal, expectedLib, msg);
			dyld::throwf("symbol '%s' not found, expected in '%s': %s",
						symbolName, expectedLib, msg);
		}
	}

	// Now walk through each segment and apply fixups
	if (context.verboseBind) {
		dyld::log("dyld: processing %u segments with chained fixups\n", startsInImage->seg_count);
	}

	for (uint32_t segIndex = 0; segIndex < startsInImage->seg_count; ++segIndex) {
		uint32_t offsetInStarts = startsInImage->seg_info_offset[segIndex];
		if (offsetInStarts == 0)
			continue;

		const dyld_chained_starts_in_segment* segInfo =
			(dyld_chained_starts_in_segment*)((uint8_t*)startsInImage + offsetInStarts);

		const macho_segment_command* segCmd = segLoadCommand(segIndex);
		if (segCmd == nullptr) {
			if (context.verboseBind) {
				dyld::log("dyld: warning: segment %u not found\n", segIndex);
			}
			continue;
		}

		// Get the actual loaded address of the segment (handles slide)
		uint8_t* segmentBase = (uint8_t*)segActualLoadAddress(segIndex);

		if (context.verboseBind) {
			dyld::log("dyld: segment %u: base=0x%lx, format=%u, page_count=%u\n",
					 segIndex, (uintptr_t)segmentBase, segInfo->pointer_format, segInfo->page_count);
		}

		// Walk through each page in the segment
		for (uint16_t pageIndex = 0; pageIndex < segInfo->page_count; ++pageIndex) {
			...
			// Calculate the address of the first fixup in this page
			uint8_t* pageBase = segmentBase + (pageIndex * segInfo->page_size);
			uint64_t* fixupLoc = (uint64_t*)(pageBase + offsetInPage);

			// Follow the chain
			bool done = false;
			while (!done) {
				uint64_t fixupValue = *fixupLoc;

				// Parse the fixup based on pointer format
				switch (segInfo->pointer_format) {
					case DYLD_CHAINED_PTR_ARM64E:
					case DYLD_CHAINED_PTR_ARM64E_USERLAND:
					case DYLD_CHAINED_PTR_ARM64E_USERLAND24: {
						// Check if it's a bind or rebase
						// ARM64E format: bit 63 = auth, bit 62 = bind
						bool isAuth = (fixupValue >> 63) & 0x1;
						bool isBind = (fixupValue >> 62) & 0x1;
						uint64_t next = (fixupValue >> 51) & 0x7FF;  // 11-bit next

						// Validate: for binds, the "zero" field (bits 16-31) should be zero
						if (isBind) {
							uint16_t zeroField = (fixupValue >> 16) & 0xFFFF;
							if (zeroField != 0 && context.verboseBind) {
								dyld::log("WARNING: bind at %p has non-zero 'zero' field: 0x%x (fixupValue=0x%llx)\n",
										 fixupLoc, zeroField, fixupValue);
							}
						}
						...
						if (isBind) {
							...
							if (segInfo->pointer_format == DYLD_CHAINED_PTR_ARM64E_USERLAND24) {
								ordinal = fixupValue & 0xFFFFFF;  // 24-bit ordinal
								addend = ((int64_t)(fixupValue << 13)) >> 44;  // 19-bit signed addend
							} else {
								ordinal = fixupValue & 0xFFFF;  // 16-bit ordinal
								addend = ((int64_t)(fixupValue << 32)) >> 45;  // 19-bit signed addend
							}

							if (ordinal >= fixupsHeader->imports_count) {
								dyld::throwf("bind ordinal %d out of range (max %d)", ordinal, fixupsHeader->imports_count);
							}

							uintptr_t targetAddr = targetAddrs[ordinal] + addend;

							if (context.verboseBind) {
								dyld::log("  bind at %p: ordinal=%u, addend=%lld, target=0x%lx, isAuth=%d\n",
										 fixupLoc, ordinal, addend, targetAddr, isAuth);
							}

#if __has_feature(ptrauth_calls)
							if (isAuth) {
								uint16_t diversity = (fixupValue >> 32) & 0xFFFF;
								bool hasAddrDiv = ((fixupValue >> 48) & 0x1) != 0;
								uint8_t key = (fixupValue >> 49) & 0x3;

								targetAddr = (uintptr_t)__builtin_ptrauth_sign_unauthenticated(
									(void*)targetAddr, key,
									hasAddrDiv ? (uint64_t)fixupLoc : diversity);
							}
#endif
							*fixupLoc = targetAddr;

							if (context.verboseBind) {
								dyld::log("dyld: fixup: %s:%p = 0x%lx\n", this->getShortName(), fixupLoc, targetAddr);
							}
						} else {
							// It's a rebase
							uint64_t target;
							...
							if (isAuth) {
								target = fixupValue & 0xFFFFFFFF;  // 32-bit target (runtime offset)
							} else {
								target = fixupValue & 0x7FFFFFFFFFFULL;  // 43-bit target
								high8 = (fixupValue >> 43) & 0xFF;
							}

							uintptr_t targetAddr;
							// For ARM64E (not USERLAND), unauth target is vmaddr, needs slide
							// For ARM64E_USERLAND/USERLAND24, target is runtime offset from mach_header
							if (segInfo->pointer_format == DYLD_CHAINED_PTR_ARM64E && !isAuth) {
								// target is vmaddr, add slide, then add high8
								targetAddr = target + fSlide;
								if (high8) {
									targetAddr |= ((uint64_t)high8 << 56);
								}
							} else {
								// target is runtime offset from mach_header
								targetAddr = (uintptr_t)mh + target;
								if (!isAuth && high8) {
									targetAddr |= ((uint64_t)high8 << 56);
								}
							}
							...
							*fixupLoc = targetAddr;
							...
							if (context.verboseBind) {
								dyld::log("  rebase at %p: target=0x%llx, targetAddr=0x%lx, isAuth=%d, high8=0x%x\n",
										 fixupLoc, target, targetAddr, isAuth, high8);
							}
						}
						...
						if (!done) {
							fixupLoc = (uint64_t*)((uint8_t*)fixupLoc + (next * 8));
						}
						break;
					}
					...
					case DYLD_CHAINED_PTR_64_OFFSET: {
						bool isBind = (fixupValue >> 63) & 0x1;
						uint64_t next = (fixupValue >> 51) & 0xFFF;  // 12-bit next

						if (isBind) {
							// Bind
							uint32_t ordinal = fixupValue & 0xFFFFFF;  // 24-bit ordinal
							int64_t addend = ((int64_t)(fixupValue << 40)) >> 56;  // 8-bit signed addend
							...
							*fixupLoc = targetAddr;
							...
							// Rebase
							uint64_t target = fixupValue & 0xFFFFFFFFF;  // 36-bit target
							uint8_t high8 = (fixupValue >> 36) & 0xFF;

							uintptr_t targetAddr;
							if (segInfo->pointer_format == DYLD_CHAINED_PTR_64) {
								// target is vmaddr, add slide after high8
								targetAddr = target + fSlide;
								targetAddr |= ((uint64_t)high8 << 56);
							} else {
								// target is runtime offset, add to mach_header
								targetAddr = (uintptr_t)mh + target;
								targetAddr |= ((uint64_t)high8 << 56);
							}
						...
						if (!done) {
							fixupLoc = (uint64_t*)((uint8_t*)fixupLoc + (next * 4));
						}
						break;
					}

					default:
						dyld::throwf("unsupported chained pointer format: %d", segInfo->pointer_format);
				}
			}
		}
	}

	dyld::log("dyld: finished applying chained fixups to %s\n", this->getPath());
}
#endif
