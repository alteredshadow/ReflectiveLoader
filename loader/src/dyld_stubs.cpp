...
// Get argc/argv/envp from the current process
extern "C" {
    int* _NSGetArgc(void);
...
static ImageLoader::LinkContext make_default_link_context() {
    ...
    // Set up argc/argv/envp for Go dylibs and other runtimes
    // Get them from the current process via _NSGetArgc/_NSGetArgv/_NSGetEnviron
    ctx.argc = *_NSGetArgc();
    ctx.argv = (const char**)*_NSGetArgv();
    ctx.envp = (const char**)*_NSGetEnviron();
    ctx.apple = NULL;  // Will be set if needed

    // Set up program vars
    static ProgramVars programVars;
    programVars.mh = NULL;  // Not the main executable
    programVars.NXArgcPtr = _NSGetArgc();
    programVars.NXArgvPtr = (const char***)_NSGetArgv();
    programVars.environPtr = (const char***)_NSGetEnviron();
    programVars.__prognamePtr = NULL;
    ctx.programVars = programVars;
